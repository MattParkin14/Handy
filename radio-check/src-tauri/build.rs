...
fn generate_tray_translations() {
    ...
    if !locales_dir.exists() {
        ...
        fs::write(Path::new(&out_dir).join("tray_translations.rs"), stub).unwrap();
        return;
    }
    ...
    let english = match translations.get("en").and_then(|v| v.as_object()) {
        Some(e) => e,
        None => {
            let stub = "// Auto-generated stub — no English tray translations found\n";
            fs::write(Path::new(&out_dir).join("tray_translations.rs"), stub).unwrap();
            return;
        }
    };

// Stub: VoxBox uses Tauri global shortcuts only.
...
pub fn start_handy_keys_recording(_app: AppHandle, _binding_id: String) -> Result<(), String> {
    Err("handy-keys not available in VoxBox; change hotkeys via the settings text field".into())
}

#[tauri::command]
#[specta::specta]
pub fn stop_handy_keys_recording(_app: AppHandle) -> Result<(), String> {
    Err("handy-keys not available in VoxBox".into())
}
